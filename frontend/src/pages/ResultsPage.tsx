  ...
  if (!state) {
    return (
      <div className="max-w-[680px] mx-auto px-6 pt-10 pb-24">
        ...
        <button
          className="font-semibold text-sm border border-[#e2e8f0] cursor-pointer px-4.5 py-2.5 rounded-lg whitespace-nowrap inline-flex items-center justify-center gap-2 transition-all duration-[120ms] bg-white text-[#0f172a] hover:bg-[#f8fafc] hover:border-[#cbd5e1] mt-3"
          onClick={() => navigate('/')}
        >
          Back to Home
        </button>
    ...
    <div className="max-w-[680px] mx-auto px-6 pt-10 pb-24">
      ...
      <div className="bg-white border border-[#e2e8f0] rounded-xl shadow-[0_1px_2px_rgba(15,23,42,.04),0_1px_3px_rgba(15,23,42,.07)] p-9.5 text-center mb-8">
        <div className="font-extrabold text-6xl leading-none tracking-[-0.03em] tabular-nums">
          {result.score}
          <span className="text-[#94a3b8] font-bold"> / {resolvedTotal}</span>
        </div>
      ...
      <div className="text-[13px] font-semibold text-[#64748b] uppercase tracking-[.07em] my-7.5">
        Question breakdown
      </div>
      ...
      <button
        className="font-semibold text-[15px] border border-transparent cursor-pointer w-full py-3.25 rounded-lg whitespace-nowrap inline-flex items-center justify-center gap-2 transition-all duration-[120ms] bg-[#2563eb] text-white hover:bg-[#1d4ed8] mt-6.5"
        onClick={() => navigate('/')}
      >
        Back to Home
      </button>
